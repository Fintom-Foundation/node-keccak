...
describe('Keccak', function(){
  ...
  describe('#digest()', function(){
    it('supports hex encoding', function(){
      ...
      assert.equal(result, new Keccak().digest('hex'));
    });

    it('supports binary encoding', function(){
      var binary = new Keccak().digest('binary');
      assert.ok(binary);
      assert.ok(binary.length > 0);
    });

    it('defaults to binary encoding', function(){
      var binary = new Keccak().digest();
      assert.ok(binary);
      assert.ok(binary.length > 0);
    });
  ...
  describe('chaining', function(){
    it('can chain', function(){
      assert.equal(
        ...
        Keccak(224).update('vlad').digest('hex')
      );
    })
  });
});
